...
class Tool:
    ...
    def execute(self, **kwargs) -> Any:
        ...
        logger.info(f"Tool {self.name} executing...")
        try:
            validated_params = self.parameters.model_validate(kwargs)
            result = self.func(**validated_params.model_dump())
            ...
            logger.error(f"Tool {self.name} validation failed: {error_message}")
            return {"error": error_message, "success": False}
        except Exception as e:
            logger.error(f"Tool {self.name} executed with error: {str(e)}")
